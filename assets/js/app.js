(()=>{
  ...
  style.textContent=`
    ...
    .ptg-nav-item{position:relative;display:flex;align-items:center;height:100%}
    .ptg-nav-link{position:relative;display:inline-flex;align-items:center;height:100%;color:#fff;font-size:14px;font-weight:500;line-height:1.4;letter-spacing:-.02em;white-space:nowrap;opacity:.95;transition:opacity .2s ease,color .2s ease}
    .ptg-nav-link:hover,.ptg-nav-item:hover>.ptg-nav-link,.ptg-nav-item:focus-within>.ptg-nav-link{color:var(--ptg-orange);opacity:1}
    .ptg-nav-link.is-active{color:var(--ptg-orange);opacity:1}
    ...
    .ptg-nav-dropdown a{display:block;padding:12px 18px;color:#333;font-size:13px;line-height:1.35;font-weight:500;letter-spacing:-.025em;transition:background .16s ease,color .16s ease}
    .ptg-nav-dropdown a:hover,.ptg-nav-dropdown a.is-active{background:#f7f7f7;color:var(--ptg-orange)}
    ...
    @media(max-width:1180px){.ptg-site-header__inner{width:calc(100% - 32px)}.ptg-site-header__nav{gap:18px}.ptg-nav-link{font-size:13px}.ptg-site-footer__inner{width:calc(100% - 32px)}.ptg-site-footer__top{grid-template-columns:minmax(280px,1.2fr) repeat(2,minmax(160px,.7fr)) minmax(250px,.9fr)}.ptg-site-footer__menu:nth-of-type(3){display:none}}
    @media(max-width:980px){
      .ptg-site-header__inner{height:78px}.ptg-site-header__logo img{width:154px}.ptg-site-header__menu{display:block}
      .ptg-site-header__nav{position:absolute;top:78px;left:0;right:0;height:auto;display:none;flex-direction:column;align-items:stretch;gap:0;margin:0;padding:12px 24px 22px;background:rgba(25,53,78,.99);border-top:1px solid rgba(255,255,255,.12)}.ptg-site-header__nav.open{display:flex}
      .ptg-nav-item{display:block;height:auto;border-bottom:1px solid rgba(255,255,255,.1)}.ptg-nav-link{width:100%;height:auto;padding:14px 0;font-size:14px}
      .ptg-nav-dropdown{position:static;width:100%;padding:0 0 10px 14px;background:transparent;border:0;box-shadow:none;opacity:1;visibility:visible;transform:none;display:none}.ptg-nav-item.open>.ptg-nav-dropdown{display:block}.ptg-nav-dropdown a{padding:10px 0;color:rgba(255,255,255,.72);font-size:13px}.ptg-nav-dropdown a:hover,.ptg-nav-dropdown a.is-active{background:transparent;color:var(--ptg-orange)}
    }
  ...
  if(footer) footer.innerHTML=`
    <footer class="ptg-site-footer"><div class="ptg-site-footer__inner">
      ...
      <div class="ptg-site-footer__office" id="office"><div class="ptg-site-footer__address"><span>OFFICE</span><p>서울 서초구 반포대로30길 32, 3층 (서초동, 트러스트힐)</p></div><div class="ptg-site-footer__legal"><p>광고책임변호사 : 채용현</p><a href="${base}/privacy/">개인정보처리방침</a><a href="mailto:yhchae@ptglaw.co.kr">이메일무단수집거부</a></div></div>
    </div></footer>`;
  ...
  if(matchMedia('(max-width:980px)').matches){
    document.querySelectorAll('[data-dropdown]>.ptg-nav-link').forEach(link=>link.addEventListener('click',e=>{const item=link.parentElement;if(!item.classList.contains('open')){e.preventDefault();document.querySelectorAll('[data-dropdown].open').forEach(x=>x!==item&&x.classList.remove('open'));item.classList.add('open');}}));
  }
})();
